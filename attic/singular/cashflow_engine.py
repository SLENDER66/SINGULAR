...
class CashflowOpportunity:
    ...
    def __post_init__(self) -> None:
        ...
        for field_name, value in (("expected_cash", self.expected_cash), ("time_to_cash_hours", self.time_to_cash_hours), ("upfront_cost", self.upfront_cost)):
            if not isfinite(value) or value < 0:
                raise ValueError(f"{field_name} must be finite and non-negative")
        for field_name, value in (("probability", self.probability), ("recurrence_score", self.recurrence_score), ("skill_fit", self.skill_fit), ("reversibility", self.reversibility), ("ownership_score", self.ownership_score)):
...
class RapidCashEngine:
    ...
    @staticmethod
    def assess(opportunity: CashflowOpportunity, objective: RapidCashObjective) -> CashflowAssessment:
        ...
        long_term_value = 0.45 * opportunity.recurrence_score + 0.35 * opportunity.ownership_score + 0.20 * opportunity.skill_fit
        downside_penalty = opportunity.upfront_cost * (1.0 - opportunity.reversibility)
        score = max(expected_value, 0.0) * (1.0 + opportunity.skill_fit) * (1.0 + long_term_value) / max(opportunity.time_to_cash_hours, 1.0) - downside_penalty
        reasons: list[str] = []
        ...
        human_review = (objective.protect_downside and opportunity.reversibility <= 0.2) or opportunity.upfront_cost > objective.target_cash * 0.25
        if human_review:
        ...
        return CashflowAssessment(opportunity.id, action, round(expected_value, 6), round(velocity, 6), round(score, 6), tuple(reasons), round(long_term_value, 6), human_review)

    @classmethod
    def rank(cls, opportunities: list[CashflowOpportunity], objective: RapidCashObjective) -> tuple[CashflowAssessment, ...]:
        assessments = [cls.assess(opportunity, objective) for opportunity in opportunities]
        return tuple(sorted(assessments, key=lambda item: (-item.score, -item.cash_velocity, item.opportunity_id)))

    @classmethod
    def build_sprint(cls, opportunities: list[CashflowOpportunity], objective: RapidCashObjective, *, max_parallel_tests: int = 3) -> tuple[CashflowAssessment, ...]:
        if max_parallel_tests <= 0:
            raise ValueError("max_parallel_tests must be positive")
        ranked = cls.rank(opportunities, objective)
        return tuple(item for item in ranked if item.action in {CashflowAction.PRIORITIZE, CashflowAction.TEST})[:max_parallel_tests]
