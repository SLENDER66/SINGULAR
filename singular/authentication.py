...
Authentication is deliberately separate from authorization/governance: a valid
principal never receives an execution capability from this module.
"""
...
def generate_master_key() -> str:
    """Generate the external Fernet key used to encrypt TOTP secrets."""
...
def _totp_ok(secret: str, code: str, now: float) -> bool:
...
class AuthenticationService:
    """Durable identity service. It has no authority to execute SINGULAR actions."""
    ...
    def confirm_totp_enrollment(self, user_id: str, code: str, *, now: float | None = None) -> tuple[str, ...]:
        ...
        with self._connect() as conn:
            ...
            if not _totp_ok(secret, code, current):
                raise AuthenticationError("authentication failed")
            conn.execute("UPDATE auth_users SET totp_ciphertext=?,pending_totp_ciphertext=NULL,pending_totp_expires=NULL,security_version=security_version+1 WHERE user_id=?", (self._fernet.encrypt(secret.encode()), user_id))
        ...
        with self._connect() as conn:
            ...
            if row["totp_ciphertext"] is not None:
                ...
                try:
                    ...
                    raise AuthenticationConfigurationError("stored authentication secret is invalid") from None
                factor_ok = _totp_ok(secret, otp or "", current) if otp else self._consume_recovery(conn, row["user_id"], recovery_code or "", current)
                if not factor_ok:
                    self._fail(conn, account, current)
                    if client is not None: self._fail(conn, client, current)
                    raise AuthenticationError("authentication failed")
    ...
    def disable_totp(self, user_id: str, password: str, *, otp: str | None = None, recovery_code: str | None = None) -> None:
        with self._connect() as conn:
            ...
            ok = _totp_ok(secret, otp or "", self._now()) if otp else self._consume_recovery(conn, user_id, recovery_code or "", self._now())
    ...
    def _limited(conn: sqlite3.Connection, subject: str, now: float) -> bool:
        ...
        if now - row["first_failure_at"] >= RATE_WINDOW:
            conn.execute("DELETE FROM auth_attempts WHERE subject_hash=?", (subject,)); return False
    ...
    def _consume_recovery(conn: sqlite3.Connection, user_id: str, code: str, now: float) -> bool:
        if not isinstance(code, str) or len(code) != 10 or not code.isascii() or not code.isalnum(): return False
