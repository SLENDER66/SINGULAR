    ...
    def check(self, mission_id: str | None = None) -> tuple[ConsistencyViolation, ...]:
        with sqlite3.connect(self.db_path) as conn:
            ...
            executions = conn.execute(
                "SELECT e.execution_key,e.mission_id,e.action_id,e.status AS execution_status, "
                "m.status AS mission_status "
                "FROM executions e JOIN mission_states m ON m.mission_id=e.mission_id"
                + mission_filter,
                params,
            ).fetchall()
            ...
            for execution in executions:
                ...
                mission_status = execution["mission_status"]

                if mission_status == "COMPLETED" and execution_status != "COMPLETED":
                    violations.append(ConsistencyViolation(
                        "MISSION_COMPLETED_WITH_NONTERMINAL_EXECUTION",
                        "Une mission COMPLETED doit avoir une exécution COMPLETED.",
                        key,
                        mid,
                    ))
                ...
                if execution_status == "COMPLETED" and mission_status != "COMPLETED":
                    violations.append(ConsistencyViolation(
                        "EXECUTION_COMPLETED_WITH_NONCOMPLETED_MISSION",
                        "Une exécution COMPLETED doit terminer la mission correspondante.",
                        key,
                        mid,
                    ))
                ...
                if execution_status == "RECOVERY_REQUIRED" and mission_status == "COMPLETED":
                    violations.append(ConsistencyViolation(
                        "RECOVERY_EXECUTION_WITH_COMPLETED_MISSION",
                        "Une exécution RECOVERY_REQUIRED ne peut pas être rattachée à une mission COMPLETED.",
                        key,
                        mid,
                    ))
                ...
                for effect in effects:
                    ...
                    if execution_status == "COMPLETED" and effect_status != "COMPLETED":
                        violations.append(ConsistencyViolation(
                            ...
                            "Une exécution d'effet externe COMPLETED doit avoir un effet externe COMPLETED.",
                            key,
                            mid,
                        ))
                    if mission_status == "COMPLETED" and effect_status != "COMPLETED":
                        violations.append(ConsistencyViolation(
                            "MISSION_COMPLETED_WITH_NONCOMPLETED_EFFECT",
                            "Une mission COMPLETED ne peut pas masquer un effet externe non terminé.",
                            key,
                            mid,
                        ))
                    if effect_status == "COMPLETED" and execution_status in {"FAILED", "RECOVERY_REQUIRED"}:
                        violations.append(ConsistencyViolation(
                            "COMPLETED_EFFECT_WITH_UNRESOLVED_EXECUTION",
                            "Un effet externe COMPLETED ne peut pas rester rattaché à une exécution FAILED ou RECOVERY_REQUIRED.",
                            key,
                            mid,
                        ))
