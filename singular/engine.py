...
class SingularEngine:
    ...
    def bottleneck(signals: dict[str, float]) -> str:
        return min(signals.items(), key=lambda item: item[1])[0] if signals else "UNKNOWN"
    ...
    def decision_assessment(decision: Decision, consequence: float = 5.0, reversibility: float = 5.0) -> DecisionAssessment:
        ...
        if decision.unknowns:
            reasons.append("Inconnues critiques présentes.")
        if irreversible:
            reasons.append("Action peu réversible.")
        if high_consequence:
            reasons.append("Conséquence potentiellement élevée.")
        if halt:
            reasons.append("HALT : validation humaine / information supplémentaire requise.")
