...
Execution remains only behind SINGULAR's validated decision boundary.
"""
...
from ..jarvis.llm import AnthropicProvider, LLMProvider
from ..jarvis.runtime import AzazelRuntime


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="azazel", description="AZAZEL governed proposal front door")
    ...
    parser.add_argument("--max-tokens", type=int, default=AzazelRuntime.DEFAULT_MAX_TOKENS)
...
def run(
    ...
    *,
    ...
    max_tokens: int = AzazelRuntime.DEFAULT_MAX_TOKENS,
    model: str | None = None,
    route: bool = False,
) -> dict[str, object]:
    """Run AZAZEL proposal/route without exposing an execution primitive."""
    selected_provider = provider or AnthropicProvider(model=model)
    runtime = AzazelRuntime(
        selected_provider,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(json.dumps({"error": "AZAZEL request failed", "type": type(exc).__name__}), file=sys.stderr)
        return 1
