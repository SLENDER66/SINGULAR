...
DEFAULT_WORKFORCE = (
    ...
    ('LIFE','Optimize organization and routines.',('life','planning'),1),
...
def build_default_registry(handlers: dict[str, Callable[[dict[str, Any]], Any]] | None = None) -> AgentRegistry:
    handlers = handlers or {}; reg = AgentRegistry()
...
class WorkforcePlanner:
    ...
    def plan(self, capabilities: list[str]) -> WorkforceResult:
        ...
        for cap in capabilities:
            ...
            if agents: selected.append(min(agents, key=lambda a: a.risk_tier).name)
