...
class PortfolioEngine:
    ...
    def _utility(assessment: OpportunityAssessment, opportunity: Opportunity) -> float:
        ...
        return opportunity.impact * opportunity.probability * (1 + opportunity.leverage / 10) * (1 + opportunity.optionality / 10) / (1 + opportunity.risk)
    ...
    def optimize(
        ...
        *,
        ...
        selections = tuple(
            ...
            for item in sorted(best[1], key=lambda candidate: (-cls._utility(assessments[candidate.id], candidate), candidate.id))
        )
