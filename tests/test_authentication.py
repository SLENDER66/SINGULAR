...
def test_password_is_hashed_and_login_is_case_normalized(tmp_path):
    ...
    with sqlite3.connect(tmp_path / "auth.db") as conn:
        salt, digest = conn.execute("SELECT password_salt,password_digest FROM auth_users").fetchone()
        token_hashes = [row[0] for row in conn.execute("SELECT token_hash FROM auth_sessions")]
    assert PASSWORD not in salt + digest
...
def test_totp_enrollment_encrypts_secret_and_requires_second_factor(tmp_path):
    ...
    assert len(codes) == 10 and all(len(code) == 16 for code in codes)
...
def test_rate_limit_is_durable_and_client_key_is_not_stored(tmp_path):
    ...
    for _ in range(5):
        with pytest.raises(AuthenticationError):
            auth.authenticate("user", "bad password 2026!", client_key="ip:1")
    with pytest.raises(RateLimited):
        auth.authenticate("user", PASSWORD, client_key="ip:1")
    with sqlite3.connect(tmp_path / "auth.db") as conn:
